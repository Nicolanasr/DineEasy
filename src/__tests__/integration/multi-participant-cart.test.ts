...
describe('Multi-Participant Cart Integration', () => {
  ...
  describe('Concurrent Cart Operations', () => {
    it('should handle multiple participants adding items simultaneously', async () => {
      ...
      await act(async () => {
        await Promise.all([
          cart1.current.handleAddItem(mockMenuItem1, 1),
          cart2.current.handleAddItem(mockMenuItem2, 1)
        ])
      })
    ...
    it('should handle quantity conflicts when multiple participants modify same item', async () => {
      ...
      await act(async () => {
        await Promise.all([
          cart1.current.handleUpdateQuantity('order-item-1', 3),
          cart2.current.handleUpdateQuantity('order-item-1', 4)
        ])
      })
    ...
    it('should sync cart state across participants via real-time updates', async () => {
      ...
      expect(cart1.current.sharedCartItems).toContainEqual(
        expect.objectContaining({ id: 'order-item-new' })
      )
      expect(cart2.current.sharedCartItems).toContainEqual(
        expect.objectContaining({ id: 'order-item-new' })
      )
    })
  ...
  describe('Cart Conflict Resolution', () => {
    it('should handle item removal conflicts', async () => {
      ...
      await act(async () => {
        await Promise.all([
          cart1.current.handleRemoveItem('order-item-1'),
          cart2.current.handleRemoveItem('order-item-1')
        ])
      })
    ...
    it('should handle cart clearing conflicts', async () => {
      ...
      await act(async () => {
        await Promise.all([
          cart1.current.handleClearCart(),
          cart2.current.handleClearCart()
        ])
      })
    ...
    it('should handle network partitions and reconnection', async () => {
      ...
      expect(result.current.sharedCartItems).toContainEqual(
        expect.objectContaining({ id: 'missed-item' })
      )
    })
  })
